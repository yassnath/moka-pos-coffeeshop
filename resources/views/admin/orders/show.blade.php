<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @endif
            @can('void', $order)
                <form method="POST" action="{{ route('admin.orders.void', $order) }}" onsubmit="return confirm('Void transaksi ini?')">
                    @csrf
                    <button type="submit" class="moka-btn-secondary border-red-300 text-red-600 hover:bg-red-50">Void</button>
                </form>
            @endcan
        </div>
    </x-slot>
    ...
    <div class="grid gap-4 xl:grid-cols-[1.3fr_1fr]">
        ...
        <x-ui.card>
            <dl class="space-y-2 text-sm text-moka-muted">
                ...
                <div class="flex items-center justify-between">
                    <dt>Metode Bayar</dt>
                    <dd class="font-semibold text-moka-ink">{{ $order->status === 'OPEN_BILL' ? '-' : $order->payment_method }}</dd>
                </div>
