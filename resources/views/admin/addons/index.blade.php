                        ...
                        @forelse ($addons as $addon)
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('admin.addons.destroy', $addon) }}" method="POST" class="inline-block" x-ref="deleteForm{{ $addon->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="ml-3 text-sm font-semibold text-red-600 hover:text-red-700" @click.prevent="openDelete($refs.deleteForm{{ $addon->id }}, @js($addon->name))">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
