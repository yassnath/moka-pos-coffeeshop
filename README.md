...
## Upload Pertama ke GitHub (Dari Project yang Sudah Ada)
Karena ini project existing, pakai alur **push existing repository**.

1. (Opsional, jika belum pernah set Git user):
```bash
...
4. Commit pertama:
```bash
git commit -m "feat: initial release moka pos coffeeshop"
```
...
6. Tambahkan remote GitHub:
```bash
git remote add origin git@github.com:yassnath/moka-pos-coffeeshop.git
```
...
### Jika remote `origin` sudah pernah ada
```bash
git remote set-url origin git@github.com:yassnath/moka-pos-coffeeshop.git
git push -u origin main
```
